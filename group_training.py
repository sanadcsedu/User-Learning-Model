...
if __name__ == '__main__':
    ...
    for experiment in range(50):
        ...
        for user in a.users:
            a.read_cur_data(user)
            accu = a.run_random_choice(user)
            # print("User: {} Precision@2 {}".format(user, accu))
            avrg_user += accu
        average_precision += (avrg_user / len(a.users))
    average_precision /= 50
    print("P@k for random choice = {}".format(average_precision))
    ...
    #         rr = a.run_roth_and_erev_v2(user)
    #         # temp[user] = rr
    #         # print(rr)
    #         mrr += rr
    #     mrr /= len(a.users)
